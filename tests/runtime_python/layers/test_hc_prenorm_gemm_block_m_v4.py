"""V4-Flash ``hc_prenorm_gemm_block_m`` (NEW kernel, M-blocked variant) test.

Spec: ``docs/mpk/deepseek_v4/vllm_kernels/hc_prenorm_gemm_block_m_tilelang.md``.

Same outputs as ``hc_prenorm_gemm`` at n_splits=1; this variant differs
only by the M-tiling factor in the upstream TileLang port. The naive
port aliases to the same device impl. Multi-batch from day 1
...
from mirage.mpk.layers.deepseek_v4.hc import V4HcPrenormGemmBlockM


def test_hc_prenorm_gemm_block_m_v4_testmode():
    device = "cuda"
    ...
    m = V4HcPrenormGemmBlockM(
        hidden_size=hidden_size,
        hc_mult=hc_mult,
        prefix="v4_hc_gemm_bm_",
    )
    ...
    print("Compiling V4HcPrenormGemmBlockM test kernel...")
    folder_path = os.path.dirname(__file__)
    pk.compile(output_dir=folder_path)

    print("Running V4HcPrenormGemmBlockM test kernel...")
    pk()
    ...
    try:
        ...
        print("PASSED: V4HcPrenormGemmBlockM compile() matches forward()")
    except AssertionError as e:
        print(
            "FAILED: V4HcPrenormGemmBlockM compile() disagrees with forward()\n"
            f"{e}"
        )
        pk.finalize()
        sys.exit(1)
...
if __name__ == "__main__":
    test_hc_prenorm_gemm_block_m_v4_testmode()
