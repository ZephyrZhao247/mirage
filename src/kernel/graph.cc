...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  // SM100 tasks
    ...
    int variant_id = task_register->register_mhc_post_sm100_task(
        customized->bgraph, params);
    task_config[op] =
        std::make_tuple(4, 1, TASK_MHC_POST_SM100, variant_id);
  } else if (name == "sum_of_squares_sm100") {
    int variant_id = task_register->register_sum_of_squares_sm100_task(
        customized->bgraph, params);
    task_config[op] =
        std::make_tuple(2, 2, TASK_SUM_OF_SQUARES_SM100, variant_id);
  ...
  } else if (name == "hash_route_lookup_sm100") {
    int variant_id = task_register->register_hash_route_lookup_sm100_task(
        customized->bgraph, params);
  ...
  } else if (name == "mla_v4_q_kv_rmsnorm_sm100") {
    int variant_id = task_register->register_mla_v4_q_kv_rmsnorm_sm100_task(
        customized->bgraph, params);
