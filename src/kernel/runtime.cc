      ...
      for (bid.y = 0; bid.y < bgraph.grid_dim.y; bid.y++) {
        for (bid.z = 0; bid.z < bgraph.grid_dim.z; bid.z++) {
          for (int subtask_id = 0; subtask_id < num_subtasks; subtask_id++) {
            ...
            if (task_type == TASK_MLA_V4_Q_KV_RMSNORM_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
            // DeepSeek V4-Flash inverse-RoPE + per-block FP8 quant on
            // attention O: grid=(num_tokens,), one CTA per token.
            if (task_type == TASK_INV_ROPE_FP8_QUANT_O_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
            // DeepSeek V4-Flash MLA prefill KV gather: grid=(T_kv, 1, 1),
            // one CTA per gathered KV row. token_offset = bid.x.
            if (task_type == TASK_MLA_V4_PREFILL_GATHER_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
            // DeepSeek V4-Flash Compressor state-update: grid=(num_tokens,),
            // one CTA per token. token_offset = bid.x; num_tokens_per_task = 1.
            if (task_type == TASK_COMPRESSOR_STATE_UPDATE_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
            // DeepSeek V4-Flash MLA prefill over gathered-KV workspace:
            // grid=(num_q_tiles,), one CTA per Q-tile. token_offset = bid.x
            // (interpreted as the q_tile_offset inside the kernel, i.e. the
            // first Q row processed by this CTA when Q_TILE > 1).
            if (task_type == TASK_MLA_V4_PREFILL_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
            // DeepSeek V4-Flash MLA decode (v1: SWA-only): grid=(num_tokens,),
            // one CTA per token. token_offset = bid.x; num_tokens_per_task = 1.
            if (task_type == TASK_MLA_V4_DECODE_SM100) {
              task.task_metadata.token_offset = bid.x;
              task.task_metadata.num_tokens_per_task = 1;
            }
