...
int TaskRegister::register_mla_v4_q_kv_rmsnorm_sm100_task(
    threadblock::Graph const &bgraph, std::vector<int> const &params) {
  // DeepSeek V4-Flash pre-attention joint Q-lora + KV-lora RMSNorm.
  //
  // Inputs:
  //   q_in       [num_tokens_total, Q_SIZE]  bf16  (split on dim 0)
  //   kv_in      [num_tokens_total, KV_SIZE] bf16  (split on dim 0)
  //   q_weight   [Q_SIZE]                    bf16  (broadcast)
  //   kv_weight  [KV_SIZE]                   bf16  (broadcast)
  // Outputs:
  //   q_out      [num_tokens_total, Q_SIZE]  bf16
  //   kv_out     [num_tokens_total, KV_SIZE] bf16
  //
  // ``Q_SIZE`` and ``KV_SIZE`` are read from the trailing dim of q_in /
  // kv_in respectively. The single ``param`` (optional) is ``eps * 1e8``
  // as an integer; if no param is supplied we default to 1e-6f. The
  // integer encoding is used because ``register_task`` only accepts
  ...
  // q_in: [num_tokens_total, Q_SIZE]
  assert(input_ops[0]->dtensor.num_dims == 2);
  int num_tokens_total = input_ops[0]->dtensor.dim[0];
  int q_size = input_ops[0]->dtensor.dim[1];
  // kv_in: [num_tokens_total, KV_SIZE]
  ...
  assert(input_ops[2]->dtensor.dim[0] == q_size);
  // kv_weight: [KV_SIZE]
  assert(input_ops[3]->dtensor.num_dims == 1);
  assert(input_ops[3]->dtensor.dim[0] == kv_size);
  // q_out: same shape as q_in
  assert(output_ops[0]->dtensor.num_dims == 2);
  assert(output_ops[0]->dtensor.dim[0] == num_tokens_total);
  assert(output_ops[0]->dtensor.dim[1] == q_size);
  // kv_out: same shape as kv_in
  assert(output_ops[1]->dtensor.num_dims == 2);
  assert(output_ops[1]->dtensor.dim[0] == num_tokens_total);
  assert(output_ops[1]->dtensor.dim[1] == kv_size);

  // Pick NUM_THREADS that divides both row widths. 128 covers V4-Flash
  // (1024 % 128 == 0, 512 % 128 == 0). Fall back to 64/32 if the user
  // gives an unusually small width (debugging / test only).
  int num_threads = 128;
  if (q_size % num_threads != 0 || kv_size % num_threads != 0) {
    num_threads = 64;
  }
  if (q_size % num_threads != 0 || kv_size % num_threads != 0) {
    num_threads = 32;
  }
  assert(q_size % num_threads == 0);
  assert(kv_size % num_threads == 0);

  mirage::transpiler::CodeKeeper code;
  code.inc_indent();
  code.e("kernel::mla_v4_q_kv_rmsnorm_task_impl<$, $, $>(",
         q_size,
         kv_size,
         num_threads);
  code.e("    task_desc->input_ptrs[0],");   // q_in
  code.e("    task_desc->input_ptrs[1],");   // kv_in
  code.e("    task_desc->input_ptrs[2],");   // q_weight
  ...
  code.e("    1e-6f);"); // eps — matches V3 rmsnorm hardwire
  // Suppress unused-params warning if the caller passed eps as a hint
  // (we ignore it in v1).
  (void)params;
  return register_task_variant(TASK_MLA_V4_Q_KV_RMSNORM_SM100,
                               code.to_string());
}
...
int TaskRegister::register_mla_v4_decode_sm100_task(
  ...
  //
  // Inputs:
  //   q          [T, NUM_HEADS, HEAD_DIM]              bf16  (broadcast;
  //                                                          kernel
  //                                                          addresses via
  //                                                          token_offset)
  //   swa_cache  [SWA_TOTAL, HEAD_DIM]                 bf16  (broadcast;
  //                                                          logically the
  //                                                          flattened
  //                                                          [num_pages,
  //                                                          page_size,
  ...
  //   attn_sink  [NUM_HEADS]                           fp32  (broadcast)
  // Outputs:
  //   o          [T, NUM_HEADS, HEAD_DIM]              bf16
  //
  // params[0]   = sliding_window (cap on attended prefix length).
  // params[1]   = ROPE_DIM (template parameter; unused in v1 math but
  //               reserved in the kernel signature so v2 can split
  //               rope/nope without changing codegen).
  // params[2]   = softmax_scale as float bits reinterpreted to int
  //               (see ``memcpy`` decode below).
  //
  // v1 simplifications: dual-cache (extra_k_cache) and topk_indices are
  // NOT plumbed yet; the Python ``compile()`` rejects those inputs.
  assert(params.size() == 3);
  int sliding_window = params[0];
  int rope_dim = params[1];
  float softmax_scale = 0.f;
  memcpy(&softmax_scale, &params[2], sizeof(float));
  ...
  int num_heads = input_ops[0]->dtensor.dim[1];
  int head_dim = input_ops[0]->dtensor.dim[2];
  // swa_cache: [SWA_TOTAL, HEAD_DIM] (2-D linearised) or
  //            [num_pages, page_size, HEAD_DIM] (3-D — flatten leading dims).
  assert(input_ops[1]->dtensor.num_dims == 2 ||
         input_ops[1]->dtensor.num_dims == 3);
  int swa_total = (input_ops[1]->dtensor.num_dims == 2)
                      ? input_ops[1]->dtensor.dim[0]
                      : input_ops[1]->dtensor.dim[0] *
                            input_ops[1]->dtensor.dim[1];
  int swa_head_dim =
      input_ops[1]
          ->dtensor.dim[input_ops[1]->dtensor.num_dims - 1];
  assert(swa_head_dim == head_dim);
  (void)swa_total;
  // positions: [T]
  assert(input_ops[2]->dtensor.num_dims == 1);
  assert(input_ops[2]->dtensor.dim[0] == num_tokens_total);
  ...
  // o: [T, NUM_HEADS, HEAD_DIM]
  assert(output_ops[0]->dtensor.num_dims == 3);
  assert(output_ops[0]->dtensor.dim[0] == num_tokens_total);
  assert(output_ops[0]->dtensor.dim[1] == num_heads);
  assert(output_ops[0]->dtensor.dim[2] == head_dim);

  assert(rope_dim >= 0 && rope_dim <= head_dim);

  // 256 threads/CTA — Blackwell convention. Lower to 128 if head_dim is
  // smaller than 128 so reductions still fan out cleanly.
  int num_threads = 256;
  if (head_dim < 128) {
    num_threads = 128;
  }
  ...
  mirage::transpiler::CodeKeeper code;
  code.inc_indent();
  code.e("kernel::mla_v4_decode_task_impl<$, $, $, $>(",
         num_heads,
         head_dim,
         rope_dim,
         num_threads);
  code.e("    task_desc->input_ptrs[0],");   // q
  code.e("    task_desc->input_ptrs[1],");   // swa_cache
  code.e("    task_desc->input_ptrs[2],");   // positions
  code.e("    task_desc->input_ptrs[3],");   // attn_sink
  code.e("    task_desc->output_ptrs[0],");  // o
  code.e("    task_desc->task_metadata.token_offset,");
  code.e("    $,", num_tokens_total);
...
int TaskRegister::register_indexer_q_transform_sm100_task(
  ...
  int const num_outputs_iq = 2;
  assert(bgraph.operators.size() == (size_t)num_inputs_iq + num_outputs_iq);
  for (auto const &op : bgraph.operators) {
    assert(op->op_type == mirage::type::TB_INPUT_OP);
    if (input_ops_iq.size() < (size_t)num_inputs_iq) {
      input_ops_iq.push_back(static_cast<tb::TBInputOp *>(op));
    } else {
  ...
  mirage::transpiler::CodeKeeper code_iq;
  code_iq.inc_indent();
  ...
  code_iq.e("    task_desc->input_ptrs[1],");
  code_iq.e("    task_desc->input_ptrs[2],");
  code_iq.e("    task_desc->input_ptrs[3],");
  code_iq.e("    task_desc->output_ptrs[0],");
  code_iq.e("    task_desc->output_ptrs[1],");
  code_iq.e("    task_desc->task_metadata.token_offset,");
  code_iq.e("    1,");
