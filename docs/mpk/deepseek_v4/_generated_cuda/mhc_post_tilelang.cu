...
// Source: deps/vllm/vllm/model_executor/layers/mhc.py (`mhc_post_tilelang`)
// Shape constants used: hc=4, hidden=4096, n_thr=128, h_blk=1024.
  ...
  *(float4*)(c_local + 0) = *(float4*)(c + (((int64_t)((int)blockIdx.x)) * (int64_t)4));
  ...
  for (int i0_h = 0; i0_h < 2; ++i0_h) {
    ...
    for (int i_3 = 0; i_3 < 32; ++i_3) {
      b_local[i_3] = ((float)((bfloat16_t*)buf_dyn_shmem)[(((i0_h * 4096) + (i_3 * 128)) + ((int)threadIdx.x))]);
    }
    __syncthreads();
    #pragma unroll
    ...
    for (int i_5 = 0; i_5 < 8; ++i_5) {
      d_local[i_5] = ((float)((bfloat16_t*)buf_dyn_shmem)[((((i0_h * 1024) + (i_5 * 128)) + ((int)threadIdx.x)) + 8192)]);
    }
    __syncthreads();
  ...
  for (int i_9 = 0; i_9 < 32; ++i_9) {
    b_local[i_9] = ((float)((bfloat16_t*)buf_dyn_shmem)[((i_9 * 128) + ((int)threadIdx.x))]);
  }
  #pragma unroll
  for (int i_10 = 0; i_10 < 8; ++i_10) {
    d_local[i_10] = ((float)((bfloat16_t*)buf_dyn_shmem)[(((i_10 * 128) + ((int)threadIdx.x)) + 8192)]);
  }
  #pragma unroll
  ...
  for (int i_14 = 0; i_14 < 32; ++i_14) {
    b_local[i_14] = ((float)((bfloat16_t*)buf_dyn_shmem)[(((i_14 * 128) + ((int)threadIdx.x)) + 4096)]);
  }
  #pragma unroll
  for (int i_15 = 0; i_15 < 8; ++i_15) {
    d_local[i_15] = ((float)((bfloat16_t*)buf_dyn_shmem)[(((i_15 * 128) + ((int)threadIdx.x)) + 9216)]);
  }
  #pragma unroll
