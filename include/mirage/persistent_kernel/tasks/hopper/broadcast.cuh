...
/**
 * NVSHMEM tile-based broadcast for Hopper+ (SM >= 90).
 * All PEs call this function symmetrically. The calling PE broadcasts its
 * src tile to all dst tensors in the team.
 *
 * input:  per-PE data, shape (BATCH_SIZE, OUTPUT_SIZE), stride OUTPUT_STRIDE
 * output: broadcast result, shape (BATCH_SIZE, OUTPUT_SIZE), stride OUTPUT_STRIDE
 */
template <typename T, int BATCH_SIZE, int OUTPUT_SIZE, int OUTPUT_STRIDE>
__device__ __forceinline__ void nvshmem_tile_broadcast(void *input_ptr,
                                                       void *output_ptr,
                                                       void *_teams,
                                                       int task_offset,
                                                       int active_tokens) {
